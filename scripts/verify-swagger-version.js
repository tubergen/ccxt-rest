...
function verify_swagger_version() {
  const swagger_yaml = yaml.load(
    fs.readFileSync("./api/swagger/swagger.yaml"),
    "utf8"
  );
  ...
  if (package_version != swagger_version) {
    ...
    console.error(
      "* swagger.yaml version does NOT match package.json. Update swagger.yaml's version to match that of package.json *"
    );
    console.error(
      "******************************************************************************************************************"
    );
