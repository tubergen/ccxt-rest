...
function handleError(req, res, label, error) {
  ...
  if (error instanceof ccxt.AuthenticationError) {
    ...
    error instanceof ccxtRestErrors.AuthError
  ) {
    res.status(403).send();
  } else if (
    error instanceof ccxt.OrderNotFound ||
    error instanceof ccxtRestErrors.UnknownExchangeNameError
  ) {
    res.status(404).send();
  } else if (
    ...
    error instanceof ccxtRestErrors.UnsupportedApiError
  ) {
    res.status(501).send();
...
async function execute(
  req,
  res,
  parameterNamesOrParameterValuesExtractor,
  functionName,
  responseTransformer
) {
  try {
    ...
    if (typeof parameterNamesOrParameterValuesExtractor === "function") {
      ...
      parameterValues = parameterNamesOrParameterValuesExtractor.map(
        (parameterName) => req.swagger.params[parameterName].value
      );
    ...
    if (exchange.has[functionName] === false) {
      throw new ccxtRestErrors.UnsupportedApiError(
        `${exchange.name}#${functionName} is not supported`
      );
    } else {
      exchange[functionName]
        .apply(exchange, parameterValues)
        .then((response) => {
...
async function getExchangeFromRequest(req) {
  ...
  let exchange;
  if (exchangeId) {
    exchange = await exchangeService.findByExchangeNameAndExchangeId(
      exchangeName,
      exchangeId
    );
  } else {
...
function getExchangeName(req) {
  const exchangeName =
    req.swagger.params.exchangeName && req.swagger.params.exchangeName.value;
