    ...
    (req, res) => {
      ...
      try {
        ...
        if (ccxt[exchangeName]) {
          ...
          exchangeService
            .saveOrUpdate(exchangeName, ccxtParam.id, ccxtParam, exchange)
            .then((exchange) => {
              jwtHelper.sign(
          ...
          throw new ccxtRestErrors.UnknownExchangeNameError(
            `${exchangeName} is not supported`
          );
        }
      } catch (error) {
        createPrivateConnectionErrorHandler(error);
  ...
  _doExchangeSpecificOrDefault(
    ...
    (req, res) => {
      ...
      try {
        ...
        if (!exchangeId) {
          throw new ccxtRestErrors.MissingRequiredTokenError(
            `Required valid token but did not get any`
          );
        }
        const exchangeName = getExchangeName(req);

        exchangeService
          .destroy(exchangeName, exchangeId)
          .then((exchange) => {
            renderExchange(exchange, res);
          })
...
function markets(req, res) {
  _doExchangeSpecificOrDefault(req, res, "markets", (req, res) => {
    execute(req, res, [], "fetchMarkets", (response) =>
      response.map(
        (rawMarket) => new exchange_response.MarketResponse(rawMarket)
      )
    );
  });
}
...
function trades(req, res) {
  _doExchangeSpecificOrDefault(req, res, "trades", (req, res) => {
    execute(req, res, ["symbol", "since", "limit"], "fetchTrades", (response) =>
      response.map((rawTrade) => new exchange_response.TradeResponse(rawTrade))
    );
  });
}
...
function tickers(req, res) {
  _doExchangeSpecificOrDefault(req, res, "tickers", (req, res) => {
    execute(req, res, ["symbol"], "fetchTickers", (response) =>
      Object.keys(response)
        .sort()
        .map((symbol) => new exchange_response.TickerResponse(response[symbol]))
    );
  });
}
...
function fetchFundingRates(req, res) {
  _doExchangeSpecificOrDefault(req, res, "fetchFundingRates", (req, res) => {
    execute(req, res, ["symbol"], "fetchFundingRates", (response) =>
      Object.keys(response)
        .sort()
        .map((symbol) => new exchange_response.TickerResponse(response[symbol]))
    );
...
function fetchFundingRate(req, res) {
  _doExchangeSpecificOrDefault(req, res, "fetchFundingRate", (req, res) => {
    execute(req, res, ["symbol"], "fetchFundingRate", (response) =>
      Object.keys(response)
        .sort()
        .map((symbol) => new exchange_response.TickerResponse(response[symbol]))
    );
    ...
    execute(
      ...
      (req, context) => {
        ...
        const parameterValues = [
          orderPlacement.symbol,
          orderPlacement.type,
          orderPlacement.side,
          orderPlacement.amount,
          orderPlacement.price,
          orderPlacement.params,
        ];
      ...
      (rawOrderPlacementResponse, context) => {
        ...
        let response = new exchange_response.OrderResponse(
          rawOrderPlacementResponse
        );
        if (!response.symbol) {
  ...
  _doExchangeSpecificOrDefault(req, res, "cancelOrder", (req, res) => {
    execute(
      ...
      (rawOrderCancellationResponse, context) => {
        var response = new exchange_response.OrderResponse(
          rawOrderCancellationResponse
        );
        if (!response.id) {
...
function fetchOrders(req, res) {
  _doExchangeSpecificOrDefault(req, res, "fetchOrders", (req, res) => {
    execute(req, res, ["symbol", "since", "limit"], "fetchOrders", (reponse) =>
      reponse.map((rawOrder) => new exchange_response.OrderResponse(rawOrder))
    );
  });
}
...
function fetchOpenOrders(req, res) {
  _doExchangeSpecificOrDefault(req, res, "fetchOpenOrders", (req, res) => {
    execute(
      ...
      (response) =>
        response.map(
          (rawOrder) => new exchange_response.OrderResponse(rawOrder)
        )
    );
  });
}
...
function fetchMyTrades(req, res) {
  _doExchangeSpecificOrDefault(req, res, "fetchMyTrades", (req, res) => {
    execute(
      ...
      (response) =>
        response.map(
          (rawTrade) => new exchange_response.TradeResponse(rawTrade)
        )
    );
  });
}
...
function _doExchangeSpecificOrDefault(
  req,
  res,
  ...
  try {
    ...
    const override =
      ...
      exchangeConfig[exchangeName].override;
    if (override && typeof override === "function") {
      override(overrideFunctionName, req, res, defaultBehaviour);
    } else if (
      override &&
      override[overrideFunctionName] &&
      typeof override[overrideFunctionName] === "function"
    ) {
      override[overrideFunctionName](req, res, defaultBehaviour);
    } else {
